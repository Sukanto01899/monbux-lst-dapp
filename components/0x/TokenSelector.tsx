...
export default function TokenSelector({
  ...
  const selectedTokenData = tokens.find(token => token.symbol.toLowerCase() === selectedToken.toLowerCase());

  return (
    <div className={`bg-slate-100 dark:bg-slate-800 p-4 rounded-lg ${className}`}>
      <label className="block text-gray-700 dark:text-gray-200 mb-2 text-sm font-medium">{label}</label>

      <div className="flex items-center gap-3">
        {/* Token Icon */}
        {selectedTokenData && (
          <Image
            alt={selectedTokenData.logoURI}
