...
import { AFFILIATE_FEE, FEE_RECIPIENT, MONAD_TESTNET_TOKENS_BY_ADDRESS } from "../../utils/monbux/constants";
import type { PriceResponse, QuoteResponse } from "../../utils/monbux/types";
import Button from "../common/Button";
import qs from "qs";
import { formatUnits } from "viem";
import { Address, type Hex, concat, numberToHex, size } from "viem";
import {
  type BaseError,
  useSendTransaction,
  useSignTypedData,
...
export default function QuoteView({
  ...
  return (
    <div className="p-4">
      <form>
        <div className="bg-slate-200 dark:bg-gray-900 p-4 rounded-2xl mb-3">
          ...
          <div className="flex items-center text-lg sm:text-3xl text-gray-900 dark:text-white">
            ...
            <span>
              {quote &&
                quote?.sellAmount &&
                formatUnits(BigInt(quote.sellAmount), sellTokenInfo().decimals).slice(0, 6)}
            </span>
        ...
        <div className="bg-slate-200 dark:bg-gray-900 p-4 rounded-2xl mb-3">
          ...
          <div className="flex items-center text-lg sm:text-3xl text-gray-900 dark:text-white">
            <Image
              alt={MONAD_TESTNET_TOKENS_BY_ADDRESS[price.buyToken.toLowerCase()].symbol}
              className="h-9 w-9 mr-2 rounded-full"
              src={MONAD_TESTNET_TOKENS_BY_ADDRESS[price.buyToken.toLowerCase()].logoURI}
              width={9}
              height={9}
            />
            <span>
              {quote &&
                quote?.buyAmount &&
                formatUnits(BigInt(quote.buyAmount), buyTokenInfo(chainId).decimals).slice(0, 6)}
            </span>
            <div className="ml-2">{buyTokenInfo(chainId).symbol}</div>
          </div>
        </div>

        <div className="bg-slate-200 dark:bg-gray-800 p-4 rounded-sm mb-3">
          <div className="text-gray-600 dark:text-gray-300">
            {quote && quote.fees && quote.fees.integratorFee && quote.fees.integratorFee.amount
              ? "Fee: " +
                Number(formatUnits(BigInt(quote.fees.integratorFee.amount), buyTokenInfo(chainId).decimals)) +
                " " +
          ...
          {/* <div className="text-gray-600 dark:text-gray-300">
            {quote && quote?.tokenMetadata?.buyToken.buyTaxBps && quote.tokenMetadata.buyToken.buyTaxBps !== "0" && (
              <p>{buyTokenInfo(chainId).symbol + ` Buy Tax: ${formatTax(quote.tokenMetadata.buyToken.buyTaxBps)}%`}</p>
            )}
            {quote.tokenMetadata.sellToken.sellTaxBps && quote.tokenMetadata.sellToken.sellTaxBps !== "0" && (
              <p>{sellTokenInfo().symbol + ` Sell Tax: ${formatTax(quote.tokenMetadata.sellToken.sellTaxBps)}%`}</p>
            )}
          </div> */}
        </div>
      </form>
      ...
      <div className="space-y-3">
        ...
        >
          {isPending ? "Confirming..." : isConfirmed ? "Order Completed ✓" : "Place Order"}
        </Button>
